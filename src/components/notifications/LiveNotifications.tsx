...
const LiveNotifications: React.FC = () => {
  ...
  const createNotificationMessage = (data: { type: string; data: any }): Notification => {
    ...
    switch (data.type) {
      case 'buy':
        return {
          message: `${addressEnd} Bought ${formatAmountV2(data.data.ethAmount)} RBTC of ${data.data.symbol}`,
          type: 'buy',
          logo: data.data.logo
        };
      case 'sell':
        return {
          message: `${addressEnd} Sold ${formatAmountV2(data.data.ethAmount)} RBTC of ${data.data.symbol}`,
          type: 'sell',
          logo: data.data.logo
        };
      case 'tokenCreated':
