...
const Home: React.FC = () => {
  const [tokens, setTokens] = useState<PaginatedResponse<
      ...
      if (showNewTokens) {
        setTokens((prevTokens) => {
          ...
          const newUniqueTokens = newTokens.filter(
            (newToken) =>
              !prevTokens.data.some(
                (existingToken) => existingToken.id === newToken.id
              ) &&
              !displayedNewTokens.some(
                (displayedToken) => displayedToken.id === newToken.id
              )
          );
  ...
  const fetchTokens = async () => {
    ...
    try {
      ...
      if (searchQuery) {
        fetchedTokens = await searchTokens(
          searchQuery,
          currentPage,
          TOKENS_PER_PAGE
        );
    ...
    setShowNewTokens((prev) => {
      ...
      if (prev) {
        ...
        setTokens((oldTokens) => {
          ...
          const updatedTokens = {
            ...oldTokens,
            data: oldTokens.data.filter(
              (token) => !displayedNewTokens.includes(token)
            ),
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* <div className="flex items-center space-x-1 mb-4">
          <Switch
            ...
            className="data-[state=checked]:bg-[#5252FF]"
          />
          <span className="text-xs text-[#F9F9F9]">Show new tokens</span>
          {newTokensBuffer.length > 0 && !showNewTokens && (
            <span className="text-xs text-[#5252FF]">
        ...
        <div className="flex items-center space-x-1">
          <Switch
            ...
            className="data-[state=checked]:bg-[#5252FF]"
          />
          <span className="text-xs text-[#F9F9F9]">NSFW filter</span>
          {newTokensBuffer.length > 0 && !showNewTokens && (
            <span className="text-xs text-[#5252FF]">
              ({newTokensBuffer.length} new)
            </span>
        ...
        {isLoading ? (
          ...
          <div className="text-center text-[#F85454] text-xl mt-10">
            {error}
          </div>
        ) : sort === "bomper" ? (
          ...
          <div className="text-center text-white text-xs mt-10">
            No tokens created in the last hour. Check back soon.
          </div>
        ) : noLiquidityTokens ? (
          <div className="text-center text-white text-xs mt-10">
