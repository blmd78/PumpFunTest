...
const TokenBalanceItem: React.FC<TokenBalanceItemProps> = ({ tokenAddress, symbol, userAddress, onClick }) => {
  ...
  const handleAddressClick = (e: React.MouseEvent) => {
    e.stopPropagation();
    window.open(`https://rootstock.blockscout.com/address/${tokenAddress}`, '_blank');
  };
...
const ProfilePage: React.FC = () => {
  ...
  return (
    <Layout>
      <SEO 
        title={`${addressToUse ? `Profile: ${formatAddressV2(addressToUse)}` : 'Your Profile'} - likeaser`}
          ...
          {activeTab === 'held' && (
            <div>
              {tokenAddresses.length > 0 ? (
                ...
                <p className="text-[#B3AEAE] text-center text-sm sm:text-base">No tokens held</p>
              )}
            </div>
          )}
          ...
          {activeTab === 'created' && (
            <div>
              {isLoading ? (
                ...
                <p className="text-[#B3AEAE] text-center text-sm sm:text-base">No tokens created</p>
              )}
              ...
              <table className="min-w-full divide-y divide-gray-700">
                <thead className="bg-gray-900">
                  <tr>
                    ...
                    <th className="px-4 py-3 text-left text-[10px] sm:text-xs font-medium text-[#B3AEAE] uppercase tracking-wider">rBTC</th>
                ...
                <tbody className="bg-gray-800 divide-y divide-gray-700">
                  {transactions.map((tx) => (
                    <tr key={tx.id} className="hover:bg-gray-700 transition-colors duration-150">
                      ...
                      <td className="px-4 py-3 whitespace-nowrap text-[10px] sm:text-xs text-[#B3AEAE]">{formatAmountV3(tx.ethAmount)} RBTC</td>
